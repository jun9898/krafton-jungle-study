...
def load_config():
    ...
    if not config.has_option('DEFAULT', 'root_directory') or not config.has_option('DEFAULT', 'old_str_1st') \
            or not config.has_option('DEFAULT', 'old_str_2st')  or not config.has_option('DEFAULT', 'old_str_3st') \
            or not config.has_option('DEFAULT', 'new_str_1st')  or not config.has_option('DEFAULT', 'new_str_2st') \
            or not config.has_option('DEFAULT', 'new_str_3st') : 
            create_config()
            config.read(config_path)
    return config
...
def rename_files():
    ...
    # folder_path = folder_path + forder_select
    ...
    config.set('DEFAULT', 'folder_path', folder_selected)
    ...
    for file_name in os.listdir(folder_path):
        if os.path.isfile(os.path.join(folder_path,file_name)):
            if old_str_entry_1st in file_name:
                ...
                ori_file_path = os.path.join(ori_file_path, file_name)
...
def main():
    ...
    if old_str_entry_2nd:
        g_old_str_entry_1st.insert(0,old_str_entry_2nd)
    if old_str_entry_3rd:
        g_old_str_entry_1st.insert(0,old_str_entry_3rd)
